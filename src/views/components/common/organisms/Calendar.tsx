...
import EventOK from "views/components/report/reviewer/molecules/EventOK";
import jaLocale from "@fullcalendar/core/locales/ja";
import styled from "styled-components";
import { ReportWorkingStatus } from "API";
import EventProblem from "views/components/report/reviewer/molecules/EventProblem";
import EventTask from "views/components/report/reviewer/molecules/EventTask";
...
function CalendarEvent(eventInfo: EventContentArg) {
  ...
  if (status) {
    if (status === ReportWorkingStatus.OK) {
      return (
        <EventStyle>
          <EventOK title={eventInfo.event.title} />
        </EventStyle>
      );
    } else if (status === ReportWorkingStatus.InTask) {
      return (
        <EventStyle>
          <EventTask title={eventInfo.event.title} />
        </EventStyle>
      );
    } else if (status === ReportWorkingStatus.InProblem) {
      return (
        <EventStyle>
          <EventProblem title={eventInfo.event.title} />
        </EventStyle>
      );
    }
  }
