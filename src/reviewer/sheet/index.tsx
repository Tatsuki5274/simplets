...
function EvalutionScreen(props: Props) {
  ...
  sectionItems?.sort(function (a, b) {
    if (
      a.sectionCategoryName &&
      b.sectionCategoryName &&
      a.sectionCategoryName > b.sectionCategoryName
    ) {
      // 仮で設定
      return 1;
    } else {
      ...
      <SheetContext.Provider value={{ sheet: sheet, setSheet: setSheet }}>
        ...
        {(() => {
          if (sheet.statusValue === 2) {
            ...
            if (
              currentUser &&
              sheet.secondReviewers?.includes(currentUser.username) &&
              sheet.topReviewers === null
            ) {
              // 部門長のコンポーネントを返却
            ...
            sheet.statusValue === 13 &&
            currentUser &&
            sheet.topReviewers?.includes(currentUser.username)
          ) {
            // 部門長が変更可能なコンポーネントを返却
