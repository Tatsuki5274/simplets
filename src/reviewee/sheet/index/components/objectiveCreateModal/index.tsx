    ...
    (async () => {
      ...
      if (sheet && sheet.section && sheet.section.items) {
        ...
        const defaultSectionKeys =
          sheet.section.items[0] && sheet.section.items[0].id
            ? sheet.section.items[0].id
            : null;
          ...
          <Formik
            ...
            validationSchema={Yup.object({
              ...
              expDoneDate: Yup.date()
                ...
                .typeError("正しく入力してください")
                .required("必須入力です"),
              sectionKeys: Yup.string()
                .required("目標カテゴリを選択してください")
                .nullable(),
            ...
            onSubmit={async (values) => {
              ...
              if (values.sectionKeys) {
                if (sheet) {
                  ...
                  const createR = await ObjectiveDao.create(
                    createObjective,
                    createI
                  );
                  if (createR) {
