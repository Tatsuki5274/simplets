...
export async function sendEmailMutation(
  sendEmailInput: SendEmail
): Promise<GraphQLResult<APIt.SendEmailMutation>> {
  ...
  const sendR: GraphQLResult<APIt.SendEmailMutation> = (await API.graphql(
    graphqlOperation(sendEmail, sendMV)
  )) as GraphQLResult<APIt.SendEmailMutation>;
  return sendR;
}
