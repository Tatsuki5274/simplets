...
export const ReportDao = {
  create: async (
    ...
    try {
      ...
      const createR = await graphqlMutation<
        APIt.CreateReportMutationVariables,
        APIt.CreateReportMutation
      >(mutation, createMV);
      if (createR.data) {
  ...
  update: async (
    ...
    try {
      ...
      const updateR = await graphqlMutation<
        APIt.UpdateReportMutationVariables,
        APIt.UpdateReportMutation
      >(mutation, updateMV);
      if (updateR.data) {
  ...
  delete: async (
    ...
    try {
      ...
      const deleteR = await graphqlMutation<
        APIt.DeleteReportMutationVariables,
        APIt.DeleteReportMutation
      >(mutation, deleteMV);
      if (deleteR.data) {
  ...
  listSub: async (
    ...
    try {
      ...
      const listGQL = await graphqlQuery<
        APIt.ListReportsSubQueryVariables,
        APIt.ListReportsSubQuery
      >(query, listQV);
      if (listGQL.data) {
  ...
  listCompanyDate: async (
    ...
    try {
      ...
      const listGQL = await graphqlQuery<
        APIt.ListReportsCompanyDateQueryVariables,
        APIt.ListReportsCompanyDateQuery
      >(query, listQV);
      if (listGQL.data) {
        const listQ: APIt.ListReportsCompanyDateQuery = listGQL.data;
        if (
          listQ.listReportsCompanyDate &&
          listQ.listReportsCompanyDate.items
        ) {
          const gotReportsCompanyDate = listQ.listReportsCompanyDate
            .items as Report[];
  ...
  listReviewee: async (
    ...
    try {
      ...
      const listGQL = await graphqlQuery<
        APIt.ListReportsRevieweeQueryVariables,
        APIt.ListReportsRevieweeQuery
      >(query, listQV);
      if (listGQL.data) {
        const listQ: APIt.ListReportsRevieweeQuery = listGQL.data;
        if (listQ.listReportsReviewee && listQ.listReportsReviewee.items) {
          const gotReportsCompanyDate = listQ.listReportsReviewee
            .items as Report[];
          return gotReportsCompanyDate;
        } else console.error("情報の取得に失敗しました");
      } else console.error("情報の取得に失敗しました");
