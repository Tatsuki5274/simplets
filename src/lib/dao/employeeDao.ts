  ...
  create: async (
    ...
    try {
      ...
      const createR = await graphqlMutation<
        APIt.CreateEmployeeMutationVariables,
        APIt.CreateEmployeeMutation
      >(mutation, createMV);
      if (createR.data) {
  ...
  update: async (
    ...
    try {
      ...
      const updateR = await graphqlMutation<
        APIt.UpdateEmployeeMutationVariables,
        APIt.UpdateEmployeeMutation
      >(mutation, updateMV);
      if (updateR.data) {
  ...
  delete: async (
    ...
    try {
      ...
      const deleteR = await graphqlMutation<
        APIt.DeleteEmployeeMutationVariables,
        APIt.DeleteEmployeeMutation
      >(mutation, deleteMV);
      if (deleteR.data) {
  ...
  get: async (
    ...
    try {
      ...
      const getGQL = await await graphqlQuery<
        APIt.GetEmployeeQueryVariables,
        APIt.GetEmployeeQuery
      >(query, getQV);
      if (getGQL.data) {
  ...
  listCompany: async (
    ...
    try {
      ...
      const listGQL = await graphqlQuery<
        APIt.ListEmployeesCompanyQueryVariables,
        APIt.ListEmployeesCompanyQuery
      >(query, listQV);
      if (listGQL.data) {
        const listQ: APIt.ListEmployeesCompanyQuery = listGQL.data;
        if (listQ.listEmployeesCompany && listQ.listEmployeesCompany.items) {
          const gotEmployeesCompany = listQ.listEmployeesCompany
            .items as Employee[];
  ...
  listManager: async (
    ...
    try {
      ...
      const listManagerGQL = await graphqlQuery<
        APIt.ListEmployeesManagerQueryVariables,
        APIt.ListEmployeesManagerQuery
      >(query, listManagerQV);
      if (listManagerGQL.data) {
        const listManagerQ: APIt.ListEmployeesManagerQuery =
          listManagerGQL.data;
        if (
          listManagerQ.listEmployeesManager &&
          listManagerQ.listEmployeesManager.items
        ) {
          const gotEmployees = listManagerQ.listEmployeesManager
            .items as Employee[];
          return gotEmployees;
        } else console.error("情報の取得に失敗しました");
      } else console.error("情報の取得に失敗しました");
